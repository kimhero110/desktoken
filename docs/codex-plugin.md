...
2026-09-08 后续只读检查：8 个插件钩子均为 enabled=true、trust=trusted，待审核状态已解除。该检查只证明配置就绪；一次 20 秒只读观察没有捕获到新事件，因此不将其记录为插件桌面执行通过。需在已加载可信插件的新轮次/任务中验证实际事件。README 与配置窗口已加入必须审核信任的入口说明，并明确现有安装按钮生成用户钩子，插件用户不要重复安装；配置检查不代替插件信任检查。
配置提示本机部署：43 项 Node 测试通过，release 构建成功（4 分 42 秒），桌面程序备份后原位替换并重启，SHA-256 为 6697c954280fbe80130c1ec13d9de54f7ba35833351564201874ae3ced1c9ec5。未改动插件定义或信任配置，未创建新版本标签。
