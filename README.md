...
- **原生进程发现**：Antigravity 通道找语言服务器用的是系统原生接口（WMI 进程查询 + TCP 表），不拉起 PowerShell 之类的子进程
...
- 开机自启：设置里关，或删除 Startup 文件夹里的 `QuotaBar.lnk`（旧版本写在注册表 `HKCU\...\Run\QuotaBar`，仅在设置里显式开关自启动时删除匹配本程序的那条；程序平时不碰注册表）
