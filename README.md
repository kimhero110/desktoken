...
> 插件接入验证：已打包并本机安装 QuotaBar 独立插件，运行时识别 8 个插件来源钩子；旧用户接入已备份移除，8 个插件钩子已确认受信任，实际插件事件仍待验证，详见接入记录。见 [插件接入记录](docs/codex-plugin.md)。
...
> **桌面端 Hook 信任（必读）**：ChatGPT 桌面端要求先审核信任钩子才会上报任务状态——设置 → Coding → Hooks：QuotaBar 插件在「QuotaBar」条目，用户钩子在「用户配置」条目，逐项审核并信任；安装或开启开关不等于已信任，信任前对应钩子不会上报事件，钩子定义更新后可能需要重新审核。已使用 QuotaBar 插件时无需再安装用户钩子，避免重复。
