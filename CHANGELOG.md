...
- 安全：Antigravity 语言服务器发现改为原生实现（WMI 进程查询 + IP Helper TCP 表），不再拉起 PowerShell 子进程；macOS 路径不变。
- 安全：开机自启改为 Startup 文件夹快捷方式（Shell COM 写 `.lnk`），程序启动时不再写注册表——只有用户在设置里显式开关自启动才会改动。
- 迁移：旧版写在 `HKCU\...\Run\QuotaBar` 的自启项在首次成功开关自启动时自动清理；指向其他程序的同名 Run 值或同名快捷方式视为冲突，报错并保留原样，绝不覆盖。
- 依赖：新增 `wmi 0.15.2`、`windows 0.61.3`（与 beta 线一致的锁定版本），其余依赖不变。
- 说明：本版仅回移上述两项修复；不含任务监视等 beta 功能。
