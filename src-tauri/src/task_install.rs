...
// Apply Windows security at creation, before any configuration bytes exist in the new file.
// std::fs::Permissions only represents the read-only attribute on Windows.
fn create_copy_file(path: &Path, source: Option<&Path>) -> std::io::Result<std::fs::File> {
    #[cfg(windows)]
...
fn atomic(path: &Path, bytes: &[u8]) -> Result<(), String> {
    ...
    let result = (|| -> std::io::Result<()> {
        ...
        let mut f = create_copy_file(&tmp, source)?;
        created = true;
        if let Some(source) = source { f.set_permissions(std::fs::metadata(source)?.permissions())?; }
        f.write_all(bytes)?; f.sync_all()?; drop(f);
        std::fs::rename(&tmp, path)
    })();
    if result.is_err() && created { let _ = std::fs::remove_file(&tmp); }
...
fn change_locked(path: &Path, tool: &str, current: &str, install: bool) -> Result<InstallResult, String> {
    ...
    atomic(&receipt_path, &serde_json::to_vec(&Receipt {tool:tool.into(),definitions}).map_err(|e| e.to_string())?)?;
    ...
    if let Some(bytes) = &original {
        ...
        let mut f = create_copy_file(&backup, Some(path)).map_err(|e| e.to_string())?;
    ...
    match next { Some(bytes) => atomic(path, &bytes)?, None => std::fs::remove_file(path).map_err(|e| e.to_string())? }
    Ok(report)
}
...
mod tests {
    ...
    fn missing_security_source_must_not_create_copy_with_default_acl() {
        ...
        assert!(create_copy_file(&path,Some(&dir.join("missing.json"))).is_err());
        assert!(!path.exists());
    }
    #[test]
    ...
    fn atomic_failure_must_not_delete_preexisting_temp_file() {
        ...
        assert!(atomic(&path,b"replacement").is_err());
