    ...
    match std::fs::File::open(path) {
        Ok(file) => {
            ...
            if bytes.len() > 2_097_152 {
                return Err("配置文件超过 2 MiB，未修改".into());
            }
            Ok(Some(bytes))
        }
    ...
    for definition in definitions {
        ...
        for (event, groups) in events {
            for group in groups.as_array().ok_or("接入记录无效")? {
                for h in group["hooks"].as_array().ok_or("接入记录无效")? {
                    known.push((event.clone(), h.clone()));
                }
            }
        }
    }
    ...
    for (event, groups) in hooks.iter_mut() {
        ...
        for (index, group) in groups.iter_mut().enumerate() {
            if group.get("matcher").is_some_and(|m| !m.is_string()) {
                return Err("matcher 必须是字符串，未修改配置".into());
            }
            let handlers = group
                .get_mut("hooks")
                .and_then(Value::as_array_mut)
                .ok_or("hook 配置结构无效，未修改")?;
            if handlers.iter().any(|h| {
                known
                    .iter()
                    .any(|(e, owned)| e == event && h["command"] == owned["command"])
                    && !known.iter().any(|(e, owned)| e == event && h == owned)
            }) {
                return Err(
                    "已安装的 QuotaBar hook 被手动修改，未自动覆盖或移除；请先核对该条目".into(),
                );
            }
        ...
        if !emptied.is_empty() && groups.is_empty() {
    ...
    if install {
        ...
        for (event, groups) in wanted["hooks"].as_object().ok_or("生成配置无效")? {
            hooks
                ...
                .as_array_mut()
                .ok_or("事件配置无效")?
                .extend(groups.as_array().ok_or("生成配置无效")?.iter().cloned());
        }
    }
    if hooks.is_empty() && !preserve_empty_hooks {
        root.remove("hooks");
    }
...
pub fn change(
    ...
    let guard = std::fs::OpenOptions::new()
        ...
        .truncate(false)
        .open(&lock)
        .map_err(|e| format!("无法打开接入锁：{e}"))?;
    guard
        .try_lock()
        .map_err(|e| format!("无法取得接入锁，可能有安装器正在修改配置：{e}"))?;
...
fn change_locked(
    ...
    for p in [path, receipt_path.as_path()] {
        if std::fs::symlink_metadata(p).is_ok_and(|m| m.file_type().is_symlink()) {
            return Err("配置或接入记录是符号链接，请手动合并以保留链接".into());
        }
    }
    ...
    let mut definitions = match read(&receipt_path)? {
        Some(bytes) => {
            let receipt: Receipt =
                serde_json::from_slice(&bytes).map_err(|_| "接入记录损坏，未修改配置")?;
            if receipt.tool != tool {
                return Err("接入记录工具不匹配，未修改".into());
            }
            receipt.definitions
        }
        None => Vec::new(),
    };
    if !definitions.iter().any(|v| v == current) {
        definitions.push(current.into());
    }
    let next = if tool == "opencode" {
        if let Some(bytes) = &original {
            if !definitions.iter().any(|v| {
                v.replace("\r\n", "\n").as_bytes()
        ...
        if install {
            Some(current.as_bytes().to_vec())
        } else {
            None
        }
    } else {
        Some(merge(original.as_deref(), &definitions, current, install)?)
    };
    let same = if tool == "opencode" {
        original == next
    } else {
        original
            .as_deref()
    ...
    if definitions.len() > 32 {
        return Err("接入历史超过上限，请手动核对后清理接入记录".into());
    }
    ...
    if read(path)? != original {
        return Err("配置被其他程序更新，本次未写入；请重试".into());
    }
    match next {
        Some(bytes) => {
            crate::atomic_file::replace(path, &bytes, 1, 0).map_err(|e| e.to_string())?
        }
...
mod tests {
    ...
    fn protect_with_guests_read_deny(path: &Path) {
        ...
        let dacl = prefix
            .find("D:")
            .unwrap_or_else(|| panic!("fixture SDDL missing DACL marker: {current}"));
    ...
    fn check_windows_acl(protect: bool) {
        let dir = crate::settings::app_data_dir().join(if protect {
            "protected-acl"
        } else {
            "inherited-acl"
        });
        ...
        assert_eq!(
            expected.contains("D:P"),
            protect,
            "fixture inheritance mismatch"
        );
        for (command, install) in [
            ...
            assert_eq!(
                acl_sddl(&path, false),
                expected,
                "configuration ACL changed"
            );
            assert_eq!(
                acl_sddl(Path::new(report.backup.as_ref().unwrap()), false),
                expected,
                "backup ACL changed"
            );
        }
    ...
    fn active_lock_blocks_changes_and_release_allows_retry() {
        ...
        let lock = std::fs::OpenOptions::new()
            ...
            .create(true)
            .truncate(false)
            .open(path.with_extension("quotabar-lock"))
            .unwrap();
        ...
        drop(lock);
        assert!(
            change(&path, "claude", &own("receiver"), true)
                .unwrap()
                .changed
        );
    }
    ...
    fn lock_holder_process() {
        let Some(path) = std::env::var_os("QUOTABAR_TEST_LOCK_PATH") else {
            return;
        };
        let guard = std::fs::OpenOptions::new()
            .read(true)
    ...
    fn atomic_failure_must_not_delete_preexisting_temp_file() {
        let dir = crate::settings::app_data_dir().join("atomic-collision");
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("config.json");
    ...
    fn own(cmd: &str) -> String {
        serde_json::json!({"hooks":{"Stop":[{"hooks":[{"type":"command","command":cmd}]}]}})
            .to_string()
    }
    #[test]
    ...
    fn invalid_shapes_are_rejected() {
        for raw in [
            ...
            r#"{"hooks":[]}"#,
            r#"{"hooks":{"Stop":{}}}"#,
            r#"{"hooks":{"Stop":[{"matcher":42,"hooks":[]}]}}"#,
        ] {
            assert!(merge(Some(raw.as_bytes()), &[], &own("x"), true).is_err());
        }
    }
    #[test]
    ...
    fn disk_backups_idempotency_and_plugin_conflict() {
        let dir = crate::settings::app_data_dir().join("install-test");
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("settings.json");
        std::fs::write(&path, b"{\"user\":true}").unwrap();
        let a = change(&path, "claude", &own("receiver"), true).unwrap();
        assert_eq!(
            std::fs::read(a.backup.unwrap()).unwrap(),
            b"{\"user\":true}"
        );
        assert!(
            !change(&path, "claude", &own("receiver"), true)
        ...
        std::fs::write(&plugin, "user code").unwrap();
